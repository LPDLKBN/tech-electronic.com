...
$typeListUrl = "brand/".$type."/".strtolower($typeUrl0).".html";
$productUrl = get_products_url($jianjie0,$type,$typeUrl0,$pid);
$brandListUrl = get_second_url($type,$typeUrl0,strtolower($brand));
// 查询评论
	...
	<div class="product-details-area mt-20">
		<div class="container-fluid">
			<div class="product-details data_input">
								...
								<div class="product-details-desc">
									...
									<div class="product-meta">
										<ul class="list-none">
											...
											<li>Brand: 
												<a href="<?php echo $brandListUrl; ?>" title="<?php echo $brand; ?>"><?php echo $brand; ?></a> 
												<span>|</span>
											</li>
											<?php 
								...
								<div class="product-action stuck text-left">
									...
									<form action="data/cart_buy.php" method="post">
										...
										<div class="product-quantity mt-15">
											...
											<?php echo $result_arr[0]->stock=="ok"?"<span class=\"text-stock\">In Stock</span>":"<span class=\"text-onstock\">on Stock</span>"; ?>
										</div>
				...
				<?php if(!preg_match('/A/i',$type)&&!preg_match('/B218/i',$type)&&!preg_match('/B219/i',$type)){ //只有电池才显示有电容电压链接 ?>
				<div class="row">
					<div class="col-lg-12">
						<div class="detail-attr-nav">
							<em>Capacity</em>
							<?php 
								$result_attr=json_decode(Products::GetAttrDlDyNav($type,$brand,"","dladd"));
								if (count($result_attr)!=0 && count($result_arr)!=0) {
							?>
							<ul class="detail-attr clearfix">
								<?php  
								foreach ($result_attr as $rowbt) {
									$dladd=trim($rowbt->dladd);
									if ($dladd!="") {
										$dladd=strtolower(str_replace(" ", "", $dladd));
								?>
								<li><a href="<?php echo $type."/".$typename."/".$dladd."-1"; ?>.html?brand=<?php echo $brand; ?>"><?php echo $dladd; ?></a></li>
								<?php } } ?>
							</ul>
							<?php } ?>
							<?php  
								$result_attr=json_decode(Products::GetAttrDlDyNav($type,$brand,"","dyadd"));
								if (count($result_attr)!=0 && count($result_arr)!=0) {
							?>
							<em>Voltage</em>
							<ul class="detail-attr clearfix">
								<?php  
								foreach ($result_attr as $rowbt) {
									$dyadd=trim($rowbt->dyadd);
									if ($dyadd!="") {
										$dyadd=strtolower(str_replace(" ", "", $dyadd));
								?>
								<li><a href="<?php echo $type."/".$typename."/".$dyadd."-1"; ?>.html?brand=<?php echo $brand; ?>"><?php echo $dyadd; ?></a></li>
								<?php } }?>
							</ul>
							<?php } ?>
						</div>
